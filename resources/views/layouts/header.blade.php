          ...
          <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow messages">
            ...
            @foreach($messages as $message)
              <li class="message-item">
                <a href="#">
                  @if($message->messages->photo && file_exists('storage/'.$message->messages->photo))
                    <img src="{{asset('storage/'.$message->messages->photo)}}" class="rounded-circle">
                  @else
                    <img src="{{asset('assets/img/profile-img.jpg')}}" class="rounded-circle">
                  @endif
                  <div>
                    <h4>{{$message->messages->name}}</h4>
                    <p>{{$message->message}}</p>
